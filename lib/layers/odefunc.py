...
def _get_minibatch_jacobian(y, x):
    ...
    for j in range(y.shape[1]):
        dy_j_dx = torch.autograd.grad(y[:, j],
                                      x,
                                      torch.ones_like(y[:, j]),
                                      retain_graph=True,
                                      create_graph=True)[0].view(
                                          x.shape[0], -1)
...
class ODEnet(nn.Module):
    ...
    def __init__(self,
        ...
        for dim_out, stride in zip(hidden_dims + (input_shape[0], ), strides):
            ...
            if stride == 2:
                hidden_shape[1], hidden_shape[
                    2] = hidden_shape[1] // 2, hidden_shape[2] // 2
            elif stride == -2:
                hidden_shape[1], hidden_shape[
                    2] = hidden_shape[1] * 2, hidden_shape[2] * 2
...
class AutoencoderDiffEqNet(nn.Module):
    ...
    def __init__(self,
        ...
        assert layer_type in ("ignore", "hyper", "concat", "concatcoord",
                              "blend")
    ...
    def forward(self, t, states):
        ...
        with torch.set_grad_enabled(True):
            ...
            if not self.training and dy.view(dy.shape[0], -1).shape[1] == 2:
                ...
                divergence = self.divergence_fn(dy, y,
                                                e=self._e).view(batchsize, 1)
        if self.residual:
            dy = dy - y
            divergence -= torch.ones_like(divergence) * torch.tensor(
                np.prod(y.shape[1:]), dtype=torch.float32).to(divergence)
        return tuple(
            [dy, -divergence] +
...
class AutoencoderODEfunc(nn.Module):
    ...
    def forward(self, t, y_and_logpy):
        ...
        with torch.set_grad_enabled(True):
            ...
            e_vjp_dhdy = torch.autograd.grad(h, y, self._e,
                                             create_graph=True)[0]
            e_vjp_dfdy = torch.autograd.grad(dy,
                                             h,
                                             e_vjp_dhdy,
                                             create_graph=True)[0]
            divergence = torch.sum((e_vjp_dfdy * self._e).view(batchsize, -1),
                                   1,
                                   keepdim=True)
